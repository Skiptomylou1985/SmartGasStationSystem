...
typedef struct TH_PlateIDCfg
{
	...
	unsigned char bMovingImage;			//   0、 图像模式 1、 视频模式

	unsigned char bIsNight;        //模式  0 、白天模式   1、夜间模式   
	unsigned char nImageFormat;    //图像的格式   
									// 0、ImageFormatRGB			
									// 1、ImageFormatBGR			
									// 2、ImageFormatYUV422		
	...
	int nFastMemorySize;				// DSP等的片内内存内存的大小
	...
	unsigned char bLeanCorrection;		// 是否开启倾斜矫正功能，默认开启 0、关闭矫正  1、开启矫正  
...
typedef struct TH_PlateIDResult 
{
	...
	unsigned char nCarType;			//车辆类型，已弃用
